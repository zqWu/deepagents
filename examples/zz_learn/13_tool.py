...
def download_file_in_sandbox(url: str, save_path: str) -> str:
    ...
    try:
        ...
        # 使用 wget 下载文件
        download_cmd = f"wget -O {save_path} {url}"  # 依赖沙盒
        result = backend.execute(download_cmd)

        if result.exit_code == 0:
            return f"文件已成功下载到沙盒: {save_path}"
        else:
            return f"下载失败: {result.output}"

    except Exception as e:
        return f"下载失败: {str(e)}"


@tool(description="下载文件到")
def download_file_via_local(url: str, save_path: str, runtime) -> str:
    """ 先本地下载, 再传递到沙盒 """
    import tempfile
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            ...
            tmp_file.flush()
            print(f"下载到本地成功 {tmp_file}")

            # 3. 上传到沙盒
            with open(tmp_file.name, 'rb') as f:
                content = f.read()

            upload_responses = backend.upload_files([(save_path, content)])

            # 4. 检查上传结果
            if upload_responses and upload_responses[0].error is None:
                return f"文件已成功下载到沙盒: {save_path}"
            else:
                error = upload_responses[0].error if upload_responses else "未知错误"
                return f"上传失败: {error}"
...
def create_sandbox(image: str = "ubuntu:22.04") -> SandboxSync:
    ...
    sandbox = SandboxSync.create(
        image=image,  # 这里没有python环境, 测试在环境不足时, 系统是否正常工作
...
def t_skill_in_sandbox(user_prompt: str) -> None:
    sandbox = create_sandbox(image="ubuntu:dev")
